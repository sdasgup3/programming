...
bool SROA::isAllocaPromotable(AllocaInst* AI)
{
  ...
  //for (Value::use_iterator UI = AI->use_begin(), UE = AI->use_end(); UI != UE; ++UI) {
  for (User *UI : AI->users()) {
...
bool SROA::test_U1(Instruction* AI)
{
  ...
  //for (Value::use_iterator UI = G->use_begin(), UE = G->use_end(); UI != UE; ++UI) {
  for (User *UI : G->users()) {
    Instruction *I = cast<Instruction>(UI);
...
void SROA::replaceAllocaUses(Instruction* OrigInst, unsigned offset, Value* newValue) 
{
  ...
  //for (User *UI : OrigInst->users()) {
  //for (const Use &U : OrigInst->uses()) {
  //  errs() << U<< "\n";
  //  User *UI = U.getUser();
  SmallVector<User *, 16> Users(OrigInst->user_begin(), OrigInst->user_end());
  ...
  for (unsigned I = 0, E = Users.size(); I != E; ++I) {
    ...
    if (GetElementPtrInst *Inst = dyn_cast<GetElementPtrInst>(UI)) {
      ...
      if(dyn_cast<ConstantInt>(Inst->getOperand(2))->getZExtValue() == offset) {
        ...
        for (User *DI : Inst->users()) {
          DI->replaceUsesOfWith(Inst, newValue);
        }
        //Inst->eraseFromParent();
        //replaceAllocaUses(OrigInst, offset, newValue);
      } 
      continue;
    } 
    if(ICmpInst *Inst = dyn_cast<ICmpInst>(UI)) {
      #ifdef MYDEBUG
