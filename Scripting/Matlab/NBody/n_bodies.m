...
function offset_momentum( bodies, nbodies, SOLAR_MASS )
    for ii = 1:5
        for kk = 1:3
            bodies(1).v(kk) = bodies(1).v(kk)  - bodies(ii).v(kk) ...
...
function E = bodies_energy(bodies, nbodies) 
   ...
   for ii = 1:1:nbodies
      E = E + bodies(ii).mass * ( bodies(ii).v(1) * bodies(ii).v(1) + ...
            ...
            bodies(ii).v(3) ) / 2.;
      for jj = ii + 1:1:nbodies 
         for kk = 1:1:3
            dx(kk) = bodies(ii).x(kk) - bodies(jj).x(kk);
         end
...
function bodies_advance(bodies, nbodies, dt)
  ...
  for ii=1:nbodies
    for jj=ii+1:nbodies
      ...
      for kk = 1:3
        d(kk) =  bodies(ii).x(kk) - bodies(jj).x(kk);
      end
      ...
      for kk=1:3
        bodies(ii).v(kk) = bodies(ii).v(kk) - d(kk) * bodies(jj).mass * mag;
      end

      for kk = 1:3
        bodies(jj).v(kk) = bodies(jj).v(kk) + d(kk) * bodies(ii).mass * mag;
      end
    end
  end
  ...
  for ii = 1:nbodies
    for m = 1:3
      bodies(ii).x(m) = bodies(ii).x(m)  + dt * bodies(ii).v(m);
    end
  end
end
